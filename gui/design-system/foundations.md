...
- `--font-ui`: 일반 UI, 제목, 본문, 버튼, 입력. 제품 서체를 우선하고, 그 뒤에는 시스템 UI 폰트를 영문도 지원하는 CJK fallback보다 먼저 선언한다. 그래야 CJK fallback이 시스템 UI 폰트의 영문·숫자 글리프까지 대신 표시하지 않는다. CJK(중문·한글·일문) 다국어 fallback(PingFang SC/Microsoft YaHei/Pretendard/Malgun Gothic 등)과 `:lang()` 언어별 최적화를 보장한다.
