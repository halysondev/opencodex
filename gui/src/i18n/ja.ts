...
export const ja: Record<TKey, string> = {
  ...
  "zcodeDesktop.intro": "このコンピューターの ZCode Desktop で設定した Z.AI モデルを接続します。API キーの貼り付けは不要です。公式ランタイムを起動し、既存の Desktop 会話には接続しません。",
  "zcodeDesktop.connected": "Desktop 接続済み・ローカルプロトコル確認済み",
  ...
  "zcodeDesktop.consent": "このフォルダーでのファイル編集とコマンド実行を ZCode に許可します。Desktop 設定は読み取り専用です。コマンドはネットワークにアクセスできます。信頼できる公式インストールのみ接続してください。",
  "zcodeDesktop.back": "戻る",
  "zcodeDesktop.connect": "Desktop に接続",
  "zcodeDesktop.disconnect": "切断",
  "zcodeDesktop.use": "このプロバイダーを使用",
