...
describe("strict pool routing", () => {
  for (const accountPoolStrategy of ["quota", "fill-first"] as const) {
    test(`${accountPoolStrategy}: current stays until threshold, affinity and preview agree`, () => {
      ...
      expect(resolveCodexAccountForThreadDetailed("thread", cfg)).toMatchObject({ status: "selected", accountId: "a" });
      updateAccountQuota("a", 95);
      expect(previewCodexAccountForRequest("thread", cfg)).toBe("b");
      expect(resolveCodexAccountForThreadDetailed("thread", cfg)).toMatchObject({ status: "selected", accountId: "b" });
      updateAccountQuota("a", 0);
      expect(previewCodexAccountForRequest(null, cfg)).toBe("b");
      expect(resolveCodexAccountForThreadDetailed(null, cfg)).toMatchObject({ status: "selected", accountId: "b" });
    });
  }
  test("manual fill-first B remains selected when higher-priority A recovers", () => {
    ...
    expect(resolveCodexAccountForThreadDetailed(null, cfg)).toMatchObject({ status: "selected", accountId: "a" });
  });
  test("a strict-only drained pin is retired and cannot reclaim the active account after recovery", () => {
    ...
    expect(afterRecovery).toMatchObject({ status: "selected", accountId: "a" });
  });
  test("strict fill-first finishes an automatically selected account before a higher tier returns", () => {
    ...
    expect(afterRecovery).toMatchObject({ status: "selected", accountId: "b" });
    expect(resolveCodexAccountForThreadDetailed(null, cfg)).toMatchObject({ status: "selected", accountId: "a" });
  ...
  test("round-robin still rotates new threads but excludes over-threshold candidates", () => {
    ...
    expect(resolveCodexAccountForThreadDetailed("rr-3", cfg)).toMatchObject({ status: "selected", accountId: "b" });
  });
  test("all blocked or unknown never falls back to active", () => {
    updateAccountQuota("a", 100);
    expect(previewCodexAccountForRequest(null, config())).toBeNull();
    expect(resolveCodexAccountForThreadDetailed(null, config())).toMatchObject({ status: "none" });
  });
  test("manual pause survives new healthy quota", () => {
    ...
    expect(resolveCodexAccountForThreadDetailed(null, cfg)).toMatchObject({ status: "none" });
...
describe("soft threshold remainder routing", () => {
  for (const accountPoolStrategy of ["quota", "fill-first"] as const) {
    test(`${accountPoolStrategy}: one exhausted account does not strand the last five percent`, () => {
      ...
      expect(resolveCodexAccountForThreadDetailed("remainder", cfg)).toMatchObject({ status: "selected", accountId: "b" });
      for (const used of [95, 99, 99.9]) {
      ...
      expect(resolveCodexAccountForThreadDetailed("remainder", cfg)).toMatchObject({ status: "none" });
    });
    test(`${accountPoolStrategy}: no churn between remainders, but restored headroom wins`, () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        expect(resolveCodexAccountForThreadDetailed("stable", cfg)).toMatchObject({ status: "selected", accountId: "a" });
      }
      updateAccountQuota("b", 0);
      expect(previewCodexAccountForRequest("stable", cfg)).toBe("b");
      expect(resolveCodexAccountForThreadDetailed("stable", cfg)).toMatchObject({ status: "selected", accountId: "b" });
    });
  ...
  test("exhaustion selects another remainder and a fresh reset restores eligibility", () => {
    ...
    expect(resolveCodexAccountForThreadDetailed("fallback", cfg)).toMatchObject({ status: "selected", accountId: "b" });
    updateAccountQuota("a", 0);
