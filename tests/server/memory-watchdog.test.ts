...
describe("GET /api/system/memory", () => {
  ...
  test("GET system memory includes privacy-safe appOwnedBytes scalars", async () => {
    ...
    expect(Object.keys(body.appOwnedBytes.stores).sort()).toEqual([
      "antigravity_replay", "claude_debug", "crash_ring", "cursor_blobs",
      "guardrails_activity", "guardrails_compact_continuation", "guardrails_continuation",
      "image_normalize", "injection_debug", "model_cache", "native_control_replay", "provider_debug",
      "request_log", "responses_continuation",
