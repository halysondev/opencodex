  ...
  }> = [
    {
      ...
      body: {
        input: [{
          type: "message",
          role: "user",
          content: [{ type: "input_text", text: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_") }],
        }],
      },
    },
    {
      ...
      body: {
        messages: [{
          role: "user",
          content: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_"),
        }],
      },
    },
    {
      protocol: "anthropic",
      body: {
        system: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_"),
        messages: [{
          role: "user",
          content: [{ type: "text", text: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_") }],
        }],
      },
    },
  ];
...
test("Guardrails rejects placeholder expansion before constructing an oversized string", async () => {
  const prepared = await prepareGuardrailsTurn(
    config(),
    "responses",
    { input: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_") },
  );
...
test("Guardrails Chat finish flushes only the completed choice", async () => {
  const prepared = await prepareGuardrailsTurn(
    config(),
    "chat",
    { messages: [{ role: "user", content: ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_") }] },
  );
  const rewrite = guardrailsSseDemaskRewrite(prepared.turn!.state, payload => payload);
...
test("Responses continuation keeps inherited mappings across enforce policy revisions", async () => {
  const secret = ["sk", "live", "abcdefghijklmnopqrstuvwx"].join("_");
  const nextSecret = ["sk", "live", "zyxwvutsrqponmlkjihgfedc"].join("_");
