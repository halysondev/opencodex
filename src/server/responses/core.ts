...
async function handleResponsesInner(
  ...
  try {
    ...
    const completionPolicy = createResponsesCompletionPolicy(requestContext, sidecarState, transportState.adapter);
...
||||||| parent of d7544dbdb (fix(zcode): describe image inputs with the configured vision sidecar)
...
=======
  ...
  return formatErrorResponse(400, "invalid_request_error", "Non-streaming not supported by this adapter");
    ...
    if (pendingHostAdmissionLease) {
      releaseUpstreamHostAdmission(pendingHostAdmissionLease);
      releaseCodexAuthContextProbeLease(authCtx);
>>>>>>> d7544dbdb (fix(zcode): describe image inputs with the configured vision sidecar)
    }
  }
}
