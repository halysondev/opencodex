...
| Skill/Security/Credential/Social control planes | `src/server/management/config-routes.ts` dispatches to `skill-routes.ts`, `security-routes.ts`, `credential-routes.ts`, and `social-routes.ts` by pathname prefix via lazy `import()`. Feature-flagged; mutating endpoints refuse when disabled. |
