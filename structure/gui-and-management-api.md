...
| Skill/Security/Credential control planes | `src/server/management/config-routes.ts` dispatches to `skill-routes.ts`, `security-routes.ts`, and `credential-routes.ts` by pathname prefix via lazy `import()`. Feature-flagged; mutating endpoints refuse when disabled. |
