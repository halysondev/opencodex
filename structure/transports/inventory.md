...
| OAuth account failover | `src/oauth/generic-account-failover.ts`, `src/oauth/anthropic-routing.ts` | Reactive pre-output 429 recovery is presence-driven with 2+ eligible accounts. Google Antigravity additionally rotates on 401 and on a 403 whose bounded error inspection identifies `VALIDATION_REQUIRED`; those auth verdicts are account-scoped and persist separately from credentials, while flagged accounts are rechecked after cooldown through the existing bounded Antigravity quota/model probe and restored only after success. Pool and `oauthAccountFailover` flags govern proactive routing, not reactive recovery: a disabled Anthropic pool recovers through quota ordering rather than its dormant strategy, a per-provider `enabled` beats the global default in either direction, and a non-positive fill-first threshold disables proactive usage-based rotation. |
