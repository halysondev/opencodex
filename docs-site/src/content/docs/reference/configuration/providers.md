...
| `modelContextTiers?` | `Record<string, "default" \| "long_context">` | Per-model upstream context tier. For `github-copilot`, `long_context` sends `contextTier: "long_context"` and advertises a 1,000,000-token capability before `providerContextCaps` is applied. |
