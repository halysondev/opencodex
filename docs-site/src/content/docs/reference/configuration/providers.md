...
| `modelContextTiers?` | `Record<string, "default" \| "long_context">` | Per-model upstream context tier. The Providers settings UI lists every detected or configured model for providers that expose this control. For `github-copilot`, `long_context` sends `contextTier: "long_context"` and advertises a 1,000,000-token capability before `providerContextCaps` is applied; use it only for models whose upstream tier supports it. |
